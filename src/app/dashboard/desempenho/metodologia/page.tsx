...
const rules = [
  ...
  "Assuntos prioritários podem não aparecer na prova seguinte.",
  "A metodologia pode ser atualizada com novos dados.",
];

// Espelha exatamente os fatores usados em calculatePriorityScore
// (src/lib/db/scoring.ts) e no reforço do diagnóstico — nada além disso.
const weights = [
  [
    "Recorrência histórica do assunto",
    "Com que frequência o assunto apareceu nas provas anteriores do ENEM.",
  ],
  [
    "Sua taxa de erro",
    "Quanto mais você erra um assunto, mais ele sobe na fila. Sem respostas ainda, usamos um valor neutro.",
  ],
  [
    "Importância estratégica",
    "Peso editorial do assunto dentro da matriz do ENEM.",
  ],
  [
    "Dificuldade do assunto",
    "Assuntos classificados como difíceis recebem prioridade um pouco maior.",
  ],
  [
    "Autopercepção do diagnóstico",
    "As áreas que você marcou como mais difíceis no diagnóstico dão um reforço inicial, recalibrado conforme você responde questões.",
  ],
];
...
export default async function RadarMethodologyPage() {
  ...
  return (
    <div>
      <DashboardPageHeader
        title="Como calculamos as prioridades"
        description="Critérios transparentes usados para ordenar seus assuntos de estudo — sem promessa de previsão exata."
        action={
          <Link
      ...
      <section className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <Card>
          <CardHeader>
            <CardTitle>O que a priorização não faz</CardTitle>
          </CardHeader>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm leading-6 text-slate-600">
              A ordem dos assuntos prioritários soma os fatores abaixo. É uma
              regra operacional para organizar seus estudos, não uma ciência
              exata.
            </p>
            <ol className="mt-4 divide-y divide-slate-100">
              {weights.map(([label, helper], index) => (
                <li key={label} className="flex gap-3 py-2.5">
                  <span className="tnum mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-blue-50 text-xs font-semibold text-blue-700">
                    {index + 1}
                  </span>
                  <div>
